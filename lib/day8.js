...
function task2(displays) {
  const results = displays.map(({ uniqueSignalPatterns, outputValues }, index) => {
    ...
    function fillMap(map, value, pattern) {
    ...
    const map = {};
    fillMap(map, 1, uniqueNumbers[0]);
    fillMap(map, 7, uniqueNumbers[1]);
    fillMap(map, 4, uniqueNumbers[2]);
    fillMap(map, 8, uniqueNumbers[3]);
