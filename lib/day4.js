...
function task2(draws, boards) {
  let winningBoards;
  let currentDraw;
  let boardsLeft = [...boards];
  const drawsLeft = [...draws];
  while (drawsLeft.length > 0 && boardsLeft.length > 0) {
    currentDraw = drawsLeft.shift();
    boardsLeft = boardsLeft.map(board => mark(board, currentDraw));

    // todo: find all which are currently winning (not necessarily just one!!)
    winningBoards = boardsLeft.filter((board) => check(board));
    boardsLeft = boardsLeft.filter((board) => !check(board));
  }

  const unmarkedSum = sumUnmarked(winningBoards[0]);

  return unmarkedSum * currentDraw;
}

export default function () {
  const input = readFileSync('./data/day4.txt', 'utf8');
  ...
  const taskResult = task2(draws, boards);
